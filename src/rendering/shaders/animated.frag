...
void main()
{
	...
	// Light ambient
	vec3 reflected_indirect = 0.1 * diffuse * ao * u_light_directional_intensity;
	...
	vec3 reflected_diffuse  = light_direct_diffuse_lambert(n_dot_l, diffuse, u_light_directional_color);
	vec3 reflected_specular = light_direct_specular_blinn_phong_conserved(n_dot_h, specular, roughness, u_light_directional_color);
	vec3 reflected_direct   = (reflected_diffuse + reflected_specular) * u_light_directional_intensity; // * light_directional_shadow;
