	...
	, _scene(scene)
{
	...
	{
		auto& e = ecs_state.add_entity(scene_entity.archetype_id());
		auto& graph_entity = _entity_refs.emplace_back(e, scene_entity);
...
void asset::scene_hydrater::hydrate_recurse(const scene_entity& entity, asset_loader_node& graph_entity, ecs::state& ecs_state)
{
	...
	{
		auto& ecs_entity = ecs_state.add_entity(entity.archetype_id());
